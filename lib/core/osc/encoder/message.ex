defmodule OSC.Message do
  ...
  defp new(address, arguments, options) do
    %__MODULE__{address: address, arguments: arguments}
    |> OSC.Decoder.decode(options)
  end

  types = [
    {?i, :int32, quote(do: big - signed - integer - size(32))},
  ...
  defp parse_arguments([:atom | flags], bin, acc) do
    {string, rest} = OSC.Parser.parse_string(bin)
    parse_arguments(flags, rest, [String.to_existing_atom(string) | acc])
  end
...
defimpl OSC.Encoder, for: OSC.Message do
  def encode(value, options) do
    {flags, data} =
      Enum.reduce(value.arguments, {[?,], []}, fn argument, {flags, data} ->
        flag = OSC.Encoder.flag(argument)
        value = OSC.Encoder.encode(argument, options)
        {[flag | flags], [value | data]}
      end)

    [OSC.Encoder.BitString.encode(value.address, options), pad(flags), :lists.reverse(data)]
  end
